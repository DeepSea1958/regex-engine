...
static void test_comp_valid(void) {
    ...
    int rc = bench_regcomp(&prog, "abc", REG_EXTENDED);
...
static void test_comp_extended(void) {
    ...
    int rc = bench_regcomp(&prog, "a*b+c?", REG_EXTENDED);
...
static void test_comp_with_groups(void) {
    ...
    int rc = bench_regcomp(&prog, "(abc)(def)", REG_EXTENDED);
...
static void test_comp_invalid(void) {
    ...
    rc = bench_regcomp(&prog, "|abc", REG_EXTENDED);
...
static void test_exec_match(void) {
    ...
    bench_regcomp(&prog, "abc", REG_EXTENDED);

    rc = bench_regexec(&prog, "abc", 1, pmatch, 0);
...
static void test_exec_search(void) {
    ...
    /* 精确匹配失败时回退到搜索 */
    rc = bench_regexec(&prog, "xxabcyy", 1, pmatch, 0);
...
static void test_exec_nomatch(void) {
    ...
    bench_regcomp(&prog, "xyz", REG_EXTENDED);
    rc = bench_regexec(&prog, "abc", 1, pmatch, 0);
...
static void test_exec_nosub(void) {
    ...
    rc = bench_regexec(&prog, "abc", 0, NULL, REG_NOSUB);
    CHECK_INT_EQ(0, rc, "nosub: 匹配成功");

    rc = bench_regexec(&prog, "xyz", 0, NULL, REG_NOSUB);
...
static void test_exec_capture_groups(void) {
    ...
    bench_regcomp(&prog, "(abc)(def)", REG_EXTENDED);

    rc = bench_regexec(&prog, "abcdef", 4, pmatch, 0);
...
static void test_exec_null(void) {
    ...
    CHECK_INT_EQ(REG_INVARG, bench_regexec(NULL, "abc", 1, pmatch, 0),
                 "exec: NULL prog");
    CHECK_INT_EQ(REG_INVARG, bench_regexec(&prog, NULL, 1, pmatch, 0),
                 "exec: NULL string");
    CHECK_INT_EQ(REG_INVARG, bench_regexec(&prog, "abc", 0, NULL, 0),
...
static void test_exec_star(void) {
    ...
    bench_regcomp(&prog, "a*", REG_EXTENDED);

    rc = bench_regexec(&prog, "", 1, pmatch, 0);
    CHECK_INT_EQ(0, rc, "star: 空串匹配 a*");

    rc = bench_regexec(&prog, "aaa", 1, pmatch, 0);
...
static void test_exec_alter(void) {
    ...
    bench_regcomp(&prog, "cat|dog", REG_EXTENDED);

    rc = bench_regexec(&prog, "cat", 1, pmatch, 0);
    CHECK_INT_EQ(0, rc, "alter: 'cat' 匹配");

    rc = bench_regexec(&prog, "dog", 1, pmatch, 0);
    CHECK_INT_EQ(0, rc, "alter: 'dog' 匹配");

    rc = bench_regexec(&prog, "bird", 1, pmatch, 0);
...
static void test_free_after_nomatch(void) {
    ...
    regmatch_t pmatch[1];
    bench_regcomp(&prog, "xyz", REG_EXTENDED);
    bench_regexec(&prog, "abc", 1, pmatch, 0);  /* 不匹配 */
    bench_regfree(&prog);
...
static void test_error_messages(void) {
    ...
    msg = bench_regerror(REG_OK, NULL, buf, sizeof(buf));
    CHECK_STR_EQ("no error", msg, "error: REG_OK 消息");
    CHECK_STR_EQ("no error", buf, "error: REG_OK 写入 buf");

    msg = bench_regerror(REG_BADPAT, NULL, buf, sizeof(buf));
    CHECK_STR_EQ("bad regular expression", msg, "error: REG_BADPAT");

    msg = bench_regerror(REG_NOMATCH, NULL, buf, sizeof(buf));
    CHECK_STR_EQ("no match", msg, "error: REG_NOMATCH");

    msg = bench_regerror(REG_ESPACE, NULL, buf, sizeof(buf));
    CHECK_STR_EQ("out of memory", msg, "error: REG_ESPACE");

    msg = bench_regerror(REG_INVARG, NULL, buf, sizeof(buf));
...
static void test_error_buffer(void) {
    ...
    msg = bench_regerror(REG_BADPAT, NULL, tiny, sizeof(tiny));
    CHECK_TRUE(strlen(tiny) < sizeof(tiny), "error: 小缓冲截断");
    CHECK_TRUE(tiny[3] == '\0', "error: 小缓冲以 \\0 结尾");

    /* ebuffersize=0 应不写入 */
    bench_regerror(REG_BADPAT, NULL, tiny, 0);
...
static void test_error_unknown(void) {
    ...
    bench_regcomp(&prog, "(", REG_EXTENDED);  /* 故意制造错误 */
    msg = bench_regerror(999, &prog, buf, sizeof(buf));
...
static void test_empty_pattern(void) {
    regex_prog_t prog;
    int rc = bench_regcomp(&prog, "", REG_EXTENDED);
...
static void test_long_pattern(void) {
    ...
    int rc = bench_regcomp(&prog, pattern, REG_EXTENDED);
    CHECK_INT_EQ(0, rc, "boundary: 长模式编译成功");

    regmatch_t pm[1];
    rc = bench_regexec(&prog, pattern, 1, pm, 0);
...
static void test_special_chars(void) {
    ...
    bench_regcomp(&prog, "\\d+", REG_EXTENDED);
    rc = bench_regexec(&prog, "abc123def", 1, pmatch, 0);
    ...
    bench_regcomp(&prog, "[a-z]+", REG_EXTENDED);
    rc = bench_regexec(&prog, "HELLO hello WORLD", 1, pmatch, 0);
...
static void test_full_workflow(void) {
    ...
    rc = bench_regcomp(&prog, "(\\w+)@(\\w+\\.(\\w+))", REG_EXTENDED);
    CHECK_INT_EQ(0, rc, "workflow: 邮箱模式编译成功");
    CHECK_INT_EQ(3, prog.nsub, "workflow: 3 个捕获组");

    rc = bench_regexec(&prog, "contact user@example.com for info", 1, pmatch, 0);
    ...
    rc = bench_regexec(&prog, "no email here", 1, pmatch, 0);
    CHECK_INT_EQ(REG_NOMATCH, rc, "workflow: 无邮箱返回 NOMATCH");

    bench_regerror(REG_BADPAT, &prog, errbuf, sizeof(errbuf));
